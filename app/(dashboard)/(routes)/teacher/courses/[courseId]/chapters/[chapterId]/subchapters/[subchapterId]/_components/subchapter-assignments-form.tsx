...
export const SubchapterAssignmentsForm = ({ courseId, chapterId, subchapterId, items }: Props) => {
  ...
  const onSubmit = async (values: { url: string; name?: string }) => {
    try {
      await axios.post(`/api/courses/${courseId}/chapters/${chapterId}/subchapters/${subchapterId}/assignments`, values);
      toast.success("Assignment added");
      toggleEdit();
      router.refresh();
    } catch (e) {
      console.error("[SUBCH_ASSIGN_ADD_ERROR]", e);
  ...
  const onSubmitText = async () => {
    try {
      ...
      console.error("[SUBCH_ASSIGN_TEXT_ADD_ERROR]", e);
  ...
  const onDelete = async (id: string) => {
    try {
      setDeletingId(id);
      await axios.delete(`/api/courses/${courseId}/chapters/${chapterId}/subchapters/${subchapterId}/assignments/${id}`);
      toast.success("Assignment deleted");
      router.refresh();
    } catch (e) {
      console.error("[SUBCH_ASSIGN_DELETE_ERROR]", e);
      ...
      {isEditing && (
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-2">Upload assignment PDFs for this subchapter. If you provide a display name, it will be saved with the file.</div>
          </div>
